    ...
    <div class="w-full flex items-center justify-between text-xs text-slate-500 font-medium select-none" style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif;">
        ...
        <div class="flex items-center gap-1.5">
            ...
            @if ($paginator->onFirstPage())
                <span class="w-7 h-7 rounded-lg border border-slate-200 flex items-center justify-center text-slate-300 bg-slate-50/50 cursor-not-allowed opacity-60 text-xs font-bold">
                    ‹
                </span>
            @else
                <a 
                    href="{{ $paginator->previousPageUrl() }}" 
                    class="w-7 h-7 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-slate-50 hover:border-slate-300 active:scale-95 transition-all cursor-pointer text-xs font-bold shadow-2xs no-underline"
                    title="Previous page"
                >
                    ‹
                </a>
