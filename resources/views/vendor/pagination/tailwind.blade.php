        ...
        <nav role="navigation" aria-label="Pagination Navigation" class="w-full flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
                ...
                @if ($paginator->onFirstPage())
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-400 bg-slate-100 border border-slate-200 cursor-not-allowed opacity-50 select-none shadow-2xs">
                ...
                @else
                    <a 
                        ...
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 hover:text-[#c3122e] border border-slate-200 hover:border-rose-300 shadow-2xs hover:shadow-xs active:scale-95 transition-all duration-150 cursor-pointer select-none no-underline"
                    >
                    ...
                    @foreach ($pages as $p)
                        ...
                        @else
                            <a 
                                ...
                                class="w-7 h-7 flex items-center justify-center rounded-lg text-xs font-bold text-slate-700 hover:bg-white hover:text-[#c3122e] transition-all duration-150 cursor-pointer no-underline"
                            >
                                {{ $p }}
                            </a>
                        @endif
