                    ...
                    @foreach($visibleTasks as $task)
                        <div class="h-14 flex items-center relative w-full hover:bg-slate-50/60 transition-colors">
                            ...
                            @else
                                <!-- Task Progress Bar -->
                                <div style="margin-left: {{ $task['_start_col'] * 6 }}rem; width: {{ $task['_span_cols'] * 6 }}rem" 
                                     class="h-7 rounded-lg {{ $task['level'] == 1 ? 'bg-gradient-to-r from-[#c3122e] to-[#a00e24]' : ($task['level'] == 2 ? 'bg-gradient-to-r from-indigo-500 to-indigo-600' : 'bg-gradient-to-r from-emerald-500 to-emerald-600') }} shadow-xs flex items-center px-2.5 relative z-10 transition-all group overflow-hidden">
