                            ...
                            <td class="py-4 px-4 text-right whitespace-nowrap">
                                <div class="inline-flex items-center gap-1.5 justify-end">
                                    ...
                                    @if(auth()->user()?->hasRole('super_admin') || auth()->user()?->email === 'admin@nexuspm.local' || auth()->user()?->id === 1)
                                        ...
                                        <button wire:click="deleteProject({{ $project->id }})" wire:confirm="Are you sure you want to delete '{{ $project->name }}'? This will move the project to trash." class="p-1.5 rounded-xl text-rose-600 hover:text-rose-700 hover:bg-rose-50 border border-rose-200 transition-all" title="Delete Project (PMO Admin Only)">
