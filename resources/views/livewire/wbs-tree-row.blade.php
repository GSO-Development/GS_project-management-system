...
<tr class="transition-colors duration-150 group {{ $levelConfig['rowBg'] }}">
    ...
    <td class="py-3.5 px-3 align-middle">
        <div class="flex items-center gap-2.5" style="padding-left: {{ ($level - 1) * 18 }}px;">
            ...
            @if($hasChildren)
                <button 
                    ...
                    type="button" 
                    class="w-5 h-5 rounded-lg hover:bg-slate-200/70 text-slate-400 hover:text-slate-800 flex items-center justify-center transition-all cursor-pointer shrink-0 shadow-2xs"
                    title="{{ $isCollapsed ? 'Expand subtasks' : 'Collapse subtasks' }}"
                >
                    <svg class="w-3.5 h-3.5 transition-transform duration-200 {{ $isCollapsed ? '-rotate-90 text-slate-400' : 'text-slate-700' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
            @elseif($level > 1)
                <div class="w-4 flex items-center justify-center shrink-0 text-slate-300 text-xs select-none">↳</div>
            @else
            ...
            {{-- Level Icons --}}
            @if($level === 1)
                <div class="w-6.5 h-6.5 rounded-lg {{ $levelConfig['iconBg'] }} flex items-center justify-center shrink-0 text-xs">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/>
                    </svg>
                </div>
            @elseif($level === 2)
                <div class="w-5.5 h-5.5 rounded-md {{ $levelConfig['iconBg'] }} flex items-center justify-center shrink-0">
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                </div>
            @elseif($level === 3)
                <div class="w-5 h-5 rounded-md {{ $levelConfig['iconBg'] }} flex items-center justify-center shrink-0">
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                    </svg>
                </div>
            @else
                <div class="w-5 h-5 rounded-md bg-slate-100 text-slate-600 border border-slate-200/60 shadow-2xs flex items-center justify-center shrink-0">
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
            ...
            <span class="whitespace-normal break-words {{ $levelConfig['title'] }}" title="{{ $cleanTitle }}">
                {{ $cleanTitle }}
            </span>

            {{-- Subtasks Badge --}}
            @if($hasChildren)
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-slate-100 text-slate-600 border border-slate-200/70 shrink-0 shadow-2xs">
    ...
    <td class="py-3.5 px-2 text-xs align-middle whitespace-nowrap" style="width: 125px;">
        @if($item->assignedUser)
            <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-50 border border-slate-200/80 shadow-2xs max-w-[115px]" title="{{ $item->assignedUser->name }}">
                <div class="w-4 h-4 rounded-full {{ $avatarBg }} text-white text-[8.5px] font-black flex items-center justify-center shrink-0">
        ...
        @else
            <div class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-slate-400 text-xs bg-slate-50/50 border border-dashed border-slate-200 select-none" title="Unassigned">
                <span class="w-3.5 h-3.5 rounded-full border border-dashed border-slate-300 text-slate-400 flex items-center justify-center text-[9px] shrink-0 font-mono">+</span>
        ...
        @if($item->start_date)
            <div class="flex flex-col leading-tight">
                ...
                @if($item->start_time_formatted)
                    <span class="text-[10px] font-mono font-medium text-slate-400 mt-0.5">{{ $item->start_time_formatted }}</span>
                @else
                    <span class="text-[10px] font-mono text-slate-300 mt-0.5">09:00 AM</span>
                @endif
            </div>
        @else
    ...
    <td class="py-3.5 px-2 text-xs align-middle whitespace-nowrap" style="width: 110px;">
        @if($item->end_date)
            <div class="flex flex-col leading-tight">
                <span class="text-xs font-bold {{ $isOverdue ? 'text-rose-600 font-black' : 'text-slate-800' }}">
                    {{ $item->end_date->format('M d, Y') }}
                </span>
                @if($isOverdue)
                    <span class="text-[9.5px] font-mono font-extrabold text-rose-600 uppercase tracking-tight flex items-center gap-1 mt-0.5">
                        <span class="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></span>
                        <span>Overdue</span>
                    </span>
                @elseif($item->end_time_formatted)
                    <span class="text-[10px] font-mono font-medium text-slate-400 mt-0.5">{{ $item->end_time_formatted }}</span>
                @else
                    <span class="text-[10px] font-mono text-slate-300 mt-0.5">05:30 PM</span>
                @endif
            </div>
        @else
            <span class="text-slate-300 font-mono text-xs">--</span>
    ...
    <td class="py-3.5 px-1 text-center align-middle whitespace-nowrap" style="width: 95px;">
        <div class="inline-flex items-center justify-center cursor-help" title="{{ $health['reason'] }}">
            @if($health['status'] === 'red')
                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10.5px] font-black bg-rose-50 text-rose-700 border border-rose-200 shadow-2xs">
                    <span class="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></span>
                    <span>Delayed</span>
                </span>
            @elseif($health['status'] === 'amber')
                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10.5px] font-black bg-amber-50 text-amber-800 border border-amber-200 shadow-2xs">
                    <span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
                    <span>At Risk</span>
                </span>
            @elseif($health['status'] === 'green')
                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10.5px] font-black bg-emerald-50 text-emerald-700 border border-emerald-200 shadow-2xs">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    <span>On Track</span>
                </span>
            @else
                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-slate-100 text-slate-600 border border-slate-200 shadow-2xs">
    ...
    <td class="py-3.5 px-2 align-middle whitespace-nowrap" style="width: 85px;">
        <div class="flex items-center gap-2">
            <div class="w-11 h-1.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/60 shrink-0">
                <div 
                    class="h-full rounded-full transition-all duration-300"
                    style="width: {{ $progressVal }}%; {{ $progressBarColor }}"
                ></div>
            </div>
            <span class="text-xs font-black font-mono text-slate-800 w-7 text-right tabular-nums">{{ $progressVal }}%</span>
        </div>
    </td>

    <!-- 8. STATUS (Modern Interactive Dropdown) -->
    <td class="py-3.5 px-2 align-middle whitespace-nowrap" style="width: 140px;">
            ...
            <select
                ...
                class="text-xs font-bold rounded-xl pl-3 pr-7 py-1.5 border cursor-pointer focus:outline-none focus:ring-2 focus:ring-slate-900/5 transition-all shadow-2xs w-full appearance-none whitespace-nowrap
                    {{ match($currentStatus) {
                        ...
                        default        => ($isOverdue ? 'bg-rose-50 text-rose-700 border-rose-200/90' : 'bg-slate-50 text-slate-700 border-slate-200/90 hover:bg-slate-100/70'),
                    } }}"
    ...
    <td class="py-3.5 pr-4 pl-1 text-center align-middle whitespace-nowrap" style="width: 48px;">
        <div x-data="{ open: false }" class="relative inline-block text-left">
            <button 
                @click="open = !open" 
                @click.outside="open = false" 
                type="button" 
                class="w-7 h-7 rounded-xl text-slate-400 hover:text-slate-800 hover:bg-slate-100 flex items-center justify-center transition-all cursor-pointer mx-auto active:scale-95 shadow-2xs"
                title="Task Actions"
            >
