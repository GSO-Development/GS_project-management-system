        ...
        <div class="divide-y divide-slate-100">
            @forelse($notifications as $n)
                @php
                    ...
                    if ($actionType === 'project_assignment' || str_contains($lowerMsg, 'assigned as project manager') || str_contains($lowerMsg, 'project leadership') || str_contains($lowerMsg, 'designated as')) {
                        $catName = '🚀 Project Assignment';
                        $pillStyle = 'bg-amber-50 text-amber-900 border-amber-300';
                ...
                <div class="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors duration-150 {{ $isUnread ? 'bg-white border-l-4 border-l-[#c3122e]' : 'bg-slate-50/50' }}">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if($isUnread)
                                    ...
                                    <span class="text-[9px] font-black text-[#c3122e] uppercase">Action Required</span>
                                @endif
                            </div>
                    ...
                    <div class="flex items-center gap-2 self-end sm:self-center flex-shrink-0">
                        @if(($actionType === 'project_assignment' || isset($n->data['project_id'])) && $isUnread)
                            <button 
                                wire:click="acceptProjectAssignment('{{ $n->id }}', {{ $n->data['project_id'] }})"
                                type="button"
                                style="background-color: #c3122e; color: #ffffff;"
                                class="px-4 py-2 rounded-xl text-xs font-extrabold shadow-sm hover:brightness-110 transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                            >
                        ...
                        @if($isUnread && !($actionType === 'project_assignment'))
