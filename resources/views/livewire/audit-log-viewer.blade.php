<div @if($autoRefresh) wire:poll.30s @endif class="space-y-4" style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif;">
    ...
    <!-- ── 1. CLEAN EXECUTIVE PAGE HEADER (NO REDUNDANT SUBTITLE) ── -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-1">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-rose-50 border border-rose-100 flex items-center justify-center shrink-0 shadow-2xs text-[#c3122e]">
            ...
            <div class="flex items-center gap-2.5 flex-wrap">
                ...
                <button wire:click="toggleAutoRefresh" 
                        ...
                        class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold cursor-pointer transition-all border {{ $autoRefresh ? 'bg-emerald-50 text-emerald-700 border-emerald-200/80 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200' }}">
                    <span class="w-1.5 h-1.5 rounded-full {{ $autoRefresh ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400' }}"></span>
                    <span>{{ $autoRefresh ? 'Live Sync Active' : 'Paused' }}</span>
                </button>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2.5 flex-shrink-0">
            ...
            <button wire:click="exportCsv" 
                    type="button" 
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white shadow-xs hover:shadow transition-all duration-200 cursor-pointer bg-[#c3122e] hover:bg-[#a50f27] active:scale-[0.98]">
    ...
    <div class="bg-white border border-slate-200/80 rounded-2xl p-3 shadow-2xs space-y-2.5">
        <div class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-2.5">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <button wire:click="toggleSortOrder" 
                        ...
                        class="p-2 rounded-xl text-xs font-bold text-slate-700 bg-slate-50/80 hover:bg-white border border-slate-200/90 transition-all flex items-center gap-1 cursor-pointer">
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors group">
                                ...
                                <td class="py-3.5 pl-6 pr-4 align-middle">
                                    <div class="flex items-center gap-2.5 min-w-0">
                                        <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-[#c3122e] to-[#8a0b1f] text-white flex items-center justify-center text-xs font-bold flex-shrink-0 shadow-2xs">
                                            {{ strtoupper(substr($log->user->name ?? 'S', 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <span class="text-xs font-bold text-slate-900 truncate block">{{ $log->user->name ?? 'System Automated' }}</span>
                                            <span class="text-[11px] text-slate-400 font-medium truncate block">{{ $log->user->email ?? 'Background Service' }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <!-- 4. TARGET RECORD (EXACT PROJECT DIRECTORY STYLE) -->
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50/80 p-4 rounded-2xl border border-slate-100">
                        ...
                        <div>
                            ...
                            <div class="text-[11px] text-slate-500 flex items-center gap-1.5 mt-0.5 flex-wrap">
                                <span>{{ class_basename($selectedLog->record_type) }} #{{ $selectedLog->record_id ?? 'N/A' }}</span>
