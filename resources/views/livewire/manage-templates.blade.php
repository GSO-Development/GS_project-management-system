<div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6" x-data="{ viewMode: 'grid' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200/60">
        <div>
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl bg-[#fdf4f4] border border-[#faeaea] flex items-center justify-center text-[#c3122e] shadow-2xs flex-shrink-0">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                    </svg>
                </div>
                <div>
                    <h1 class="text-2xl font-black text-slate-900 tracking-tight">Project Templates</h1>
                    <p class="text-xs text-slate-500 font-medium mt-0.5">Build, standardize, and instantiate enterprise Work Breakdown Structures (WBS) across George Steuart subsidiaries.</p>
                </div>
            </div>
        </div>
        
        <!-- Primary Action Button (PMO Admin Only) -->
        @if(auth()->user()?->isSuperAdmin())
            <button wire:click="$set('showModal', true)" 
                    class="px-4 py-2.5 rounded-xl text-xs font-bold text-white bg-[#c3122e] hover:bg-[#a00e24] shadow-md shadow-[#c3122e]/20 transition-all flex items-center gap-2 cursor-pointer active:scale-95 w-full sm:w-auto justify-center">
        ...
        @else
            <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-500 bg-slate-100 border border-slate-200">
