<div class="max-w-4xl space-y-6" style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif;">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200/80">
        <div class="flex items-center gap-3.5">
            <div class="w-11 h-11 rounded-2xl bg-[#c3122e] text-white flex items-center justify-center shadow-xs shrink-0">
    ...
    @if($successToast)
        <div class="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center justify-between shadow-2xs">
            <div class="flex items-center gap-2.5">
                <div class="w-6 h-6 rounded-lg bg-emerald-500 text-white flex items-center justify-center text-xs shadow-xs font-black">✓</div>
    ...
    <div class="bg-white border border-slate-200/90 rounded-2xl shadow-2xs p-6 sm:p-8 space-y-8">
        ...
        <div>
            <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-6">
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 font-medium mt-0.5">SMTP server host, credentials, encryption protocol, and envelope details.</p>
                </div>
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">SMTP Host <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">Port <span class="text-rose-500">*</span></label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div x-data="{ showPass: false }">
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">SMTP Password / App Secret</label>
                        <div class="relative">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">Encryption Protocol <span class="text-rose-500">*</span></label>
                        ...
                        @error('smtpEncryption') <span class="text-[11px] text-rose-600 font-bold block mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">From Address <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">From Name <span class="text-rose-500">*</span></label>
        ...
        <div class="pt-6 border-t border-slate-100">
            <div class="pb-3 mb-4">
                ...
                <p class="text-[11px] text-slate-400 font-medium">Test your mail configuration to confirm live delivery.</p>
            </div>
            ...
            @error('testEmailRecipient')
                <span class="text-[11px] text-rose-600 font-bold block mt-1.5">{{ $message }}</span>
            @enderror

            @if($testMailStatus)
                <div class="mt-3.5 p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center gap-2">
            ...
            @if($testMailError)
                <div class="mt-3.5 p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-bold flex items-start gap-2">
                    <svg class="w-4 h-4 text-rose-600 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
