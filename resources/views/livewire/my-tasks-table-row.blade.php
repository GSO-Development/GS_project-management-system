...
<tr class="hover:bg-slate-50/70 transition-colors group {{ $isDone ? 'bg-emerald-50/20' : ($isOverdue ? 'bg-rose-50/15' : ($isDueToday ? 'bg-amber-50/15' : '')) }}">
    ...
    <td class="py-3 pr-5 pl-2 align-middle text-right whitespace-nowrap">
        <div class="flex items-center justify-end gap-1">
            ...
            <!-- Daily Log / Status Update -->
            @if(auth()->check() && ($task->project && $task->project->userCan(auth()->user(), 'task.log_daily', $task)))
                <a href="{{ route('daily-updates.index', ['project' => $task->project_id, 'task' => $task->id, 'create' => 1]) }}" class="w-7 h-7 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 flex items-center justify-center transition-colors cursor-pointer" title="Daily Progress Log">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                </a>
            @endif
            <!-- Report Blocker -->
