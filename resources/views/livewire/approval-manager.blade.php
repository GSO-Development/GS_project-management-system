<div>
                            ...
                            <td class="py-3.5 px-5 text-right whitespace-nowrap">
                                <div class="inline-flex items-center justify-end gap-2">
                                    @if($req->status->value === 'pending')
                                        @if($isNewProjectPlan && ($req->project?->project_manager_id === auth()->id() || auth()->user()->isSuperAdmin()))
                                            <button wire:click="review({{ $req->id }})"
                                                    class="inline-flex items-center gap-1.5 px-3.5 py-1.5 h-8.5 rounded-xl text-xs font-black text-white shadow-xs hover:shadow-sm active:scale-95 transition-all cursor-pointer whitespace-nowrap"
                                                    style="background: linear-gradient(135deg, #c3122e 0%, #8b0d1f 100%);"
                                        ...
                                        @else
                                            <button wire:click="review({{ $req->id }})"
                                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 h-8.5 rounded-xl text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 hover:text-[#c3122e] border border-slate-200 hover:border-rose-200 shadow-2xs hover:shadow-xs transition-all cursor-pointer"
                                    ...
                                    @else
                                        ...
                                        @if(($req->status->value === 'rejected' || $req->status->value === 'cancelled') && $req->project_id && (auth()->user()->isSuperAdmin() || auth()->user()->hasRole('pmo_admin')))
                                            <button wire:click="openReassignModal({{ $req->project_id }}, {{ $req->id }})"
                                                    class="inline-flex items-center gap-1.5 px-3.5 py-1.5 h-8.5 rounded-xl text-xs font-black text-amber-900 bg-amber-50 hover:bg-amber-100 border border-amber-300 shadow-2xs hover:shadow-xs transition-all cursor-pointer whitespace-nowrap"
                                                    title="Reassign Project Leadership to another Project Manager">
                                                <svg class="w-3.5 h-3.5 text-amber-700 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                                <span>Reassign PM</span>
                                            </button>
                                        @endif
                                    @endif
                                    ...
                                    @if($req->status->value === 'pending' && $req->requested_by === auth()->id() && !$isNewProjectPlan)
                                        <button wire:click="cancelRequest({{ $req->id }})"
                                                ...
                                                class="inline-flex items-center gap-1 px-3 py-1.5 h-8.5 rounded-xl text-xs font-bold text-slate-600 hover:text-amber-700 bg-white hover:bg-amber-50 border border-slate-200 hover:border-amber-200 transition-all cursor-pointer shadow-2xs"
                                    ...
                                    @if(auth()->user()->isSuperAdmin() || $req->requested_by === auth()->id())
                                        <button wire:click="openDeleteModal({{ $req->id }})"
                                                class="inline-flex items-center justify-center w-8.5 h-8.5 rounded-xl text-slate-400 hover:text-rose-600 bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 shadow-2xs hover:shadow-xs transition-all cursor-pointer flex-shrink-0"
    ...
    {{-- ═══════════════════════════════════════════════════════════════
         7. REASSIGN & RECONFIGURE PROJECT MANAGER MODAL
         ═══════════════════════════════════════════════════════════════ --}}
    @if($showReassignModal && $reassignProject)
        <div class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-3 sm:p-5 bg-slate-900/75 backdrop-blur-md">
            <div class="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 flex flex-col max-h-[92vh] my-auto">
                ...
                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-2.5 flex-shrink-0">
                    ...
                    <button wire:click="submitReassignPm" type="button" class="px-5 py-2.5 rounded-xl text-xs font-black text-white bg-[#c3122e] hover:bg-[#a90f27] shadow-md shadow-rose-900/20 transition-all cursor-pointer flex items-center gap-2">
                        <span>🚀</span>
